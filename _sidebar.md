- [🌐 nlvm.dev](https://nlvm.dev)
- [Home](/)

- **Specification**
  - [Language (specs)](docs/specs)
  - [Compiler](docs/compiler)
  - [Standard library](docs/stdlib)
  - [Virtual machine](docs/vm)

- **Guides**
  - [Showcase](docs/showcase)
  - [Tests format](docs/tests)
  - [Milestones](docs/milestones)
  - [Optimizations](docs/optimizations)

- **Review**
  - [Coherence](review/coherence)
  - [Security audit](review/security-audit)
  - [SOLID compatibility](review/architecture/solid-compatibility)
  - [DDD compatibility](review/architecture/ddd-compatibility)

- [Changelog](CHANGELOG)
